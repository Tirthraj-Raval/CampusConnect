...
export default function StudentsUploadPage() {
  ...
  const handleUpload = async (e: any) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/university/${universityId}/upload-students`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
      ...
      if (res.ok) {
        setMessage(data.message || 'Students uploaded successfully!');
      } else {
      ...
      >
        ...
        <div className="bg-gradient-to-r from-sky-500 to-emerald-500 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Upload Student Data</h1>
              <p className="text-sm text-sky-100">Bulk import students via CSV file</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          <form onSubmit={handleUpload} className="space-y-6">
            ...
            <motion.div
              ...
              className="bg-sky-50 border-l-4 border-sky-500 p-4 rounded"
            >
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-sky-800">Expected CSV Format</h3>
                  <div className="mt-1 text-xs text-sky-700">
                    <p>• First column: Student ID</p>
                    <p>• Second column: Full Name</p>
                    <p>• Third column: Email Address</p>
                    <p>• Include header row (id,name,email)</p>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <button
                ...
                className={`w-full flex justify-center items-center gap-2 bg-gradient-to-r from-sky-500 to-emerald-500 hover:from-sky-600 hover:to-emerald-600 text-white font-medium px-6 py-3 rounded-full shadow-lg transition-all ${
        ...
        <motion.div
          ...
          className="absolute top-6 right-6 bg-gradient-to-r from-sky-500 to-emerald-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center space-x-2 text-sm"
        >
          <motion.div 
